 ...
 *
 * Dialog for editing existing companies in the platform.
 * Allows updating company name, subscription tier, limits, and settings.
 */
...
import { usePlatformCompany, useUpdateCompany } from '../../hooks/usePlatform';
...
type CompanyEditDialogProps = {
  /** The company ID to edit (null if not editing) */
  companyId: string | null;
  /** Whether the dialog is open */
  open: boolean;
  /** Callback when dialog is closed */
  onClose: () => void;
  /** Callback when company is updated successfully */
  onSaved: () => void;
};
...
/**
 * Dialog for editing an existing company in the platform.
 */
export function CompanyEditDialog({
  ...
  onClose,
  onSaved,
}: CompanyEditDialogProps): React.ReactElement {
  ...
  const { data: company, isLoading } = usePlatformCompany(
    companyId ?? '',
    open && !!companyId,
  );
  ...
  useEffect(() => {
    if (company) {
      ...
      setSubscriptionTier(company.subscriptionTier);
      setMaxUsers(String(company.maxUsers));
      setMaxSessions(String(company.maxSessions));
      setSessionLimitStrategy(company.sessionLimitStrategy);
      setMfaRequired(company.mfaRequired);
      setErrors({});
      setSubmitError(null);
    }
  }, [company]);
  ...
  const isLoadingData = isLoading && open;

  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <Box sx={{ flex: 1 }}>Edit Company</Box>
          ...
          <>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
              ...
              <FormControl fullWidth>
                <InputLabel id="edit-tier-label">Subscription Tier</InputLabel>
                <Select
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                <TextField
                  ...
                  helperText={errors.maxSessions ?? 'Concurrent sessions'}
                  fullWidth
                  required
                  disabled={isSubmitting}
              ...
              <FormControl fullWidth>
                <InputLabel id="edit-session-strategy-label">
                  Session Limit Strategy
                </InputLabel>
                <Select
                  labelId="edit-session-strategy-label"
                  value={sessionLimitStrategy}
                  label="Session Limit Strategy"
                  onChange={(e: SelectChangeEvent) =>
                    setSessionLimitStrategy(
                      e.target.value as SessionLimitStrategy,
                    )
                  }
                  disabled={isSubmitting}
                >
