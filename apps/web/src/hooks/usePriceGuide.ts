...
/**
 * Hook to fetch MSI pricing data.
 */
export function useMsiPricing(msiId: string) {
  return useQuery({
    queryKey: [...priceGuideKeys.msiDetail(msiId), 'pricing'] as const,
    queryFn: () => priceGuideApi.getMsiPricing(msiId),
    enabled: !!msiId,
  });
}

/**
 * Hook to update MSI pricing for a specific office.
 */
export function useUpdateMsiPricing() {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    ...
    }) => priceGuideApi.updateMsiPricing(msiId, officeId, prices),
    onSuccess: (_, { msiId }) => {
      void queryClient.invalidateQueries({
        queryKey: [...priceGuideKeys.msiDetail(msiId), 'pricing'],
      });
    },
  });
}
...
/**
 * Hook to batch update MSI pricing for multiple offices at once.
 * Used by the wizard to save all pricing in one call.
 */
export function useBatchUpdateMsiPricing() {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    ...
    }) => priceGuideApi.batchUpdateMsiPricing(msiId, pricing),
    onSuccess: (_, { msiId }) => {
      void queryClient.invalidateQueries({
        queryKey: [...priceGuideKeys.msiDetail(msiId), 'pricing'],
      });
